...
public class IdRule implements Rule {
    ...
    @Override
    public Operation assemble(Tokenizer tokenizer) throws ParseException {
        ParserUtil.checkType(tokenizer.consume(), Token.Type.ID);
        IdOperation operation = new IdOperation(STRING_LITERAL_RULE.assemble(tokenizer).getLiteral());
